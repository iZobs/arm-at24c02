...
static ssize_t at24c02_read(struct file *filp, char __user * buf, size_t size,
			    loff_t * ppos)
{
	int i = 0;
	int transferred = 0;
	...
	if (i2c_check_functionality
	    (dev->client->adapter, I2C_FUNC_SMBUS_READ_BYTE_DATA)) {
		while (transferred < size) {
			/*s32 i2c_smbus_read_byte_data(struct i2c_client *client, u8 command); */
			/*将会调用i2c_smbus_xfer */
			ret =
			    i2c_smbus_read_byte_data(dev->client,
						     dev->current_pointer + i);
			my_buff[i++] = (unsigned char)ret;
			transferred += 1;
#if DEBUG
			printk(KERN_NOTICE
			       "the my_buff[%d] is %x,transferred is %d", i,
			       my_buff[i], transferred);
#endif
...
static ssize_t at24c02_write(struct file *filp, const char __user * buf,
			     size_t size, loff_t * ppos)
{
	...
	dev->current_pointer = *ppos;
	if (i2c_check_functionality
	    (dev->client->adapter, I2C_FUNC_SMBUS_BYTE_DATA)) {
		...
		while (transferred < size) {
			ret =
			    i2c_smbus_write_byte_data(dev->client,
						      ...
						      my_buff[i]);
#if DEBUG
			printk(KERN_NOTICE "write data %d-----my_buff%x", i,
			       my_buff[i]);
#endif
...
static int at24c02_ioctl(struct inode *inodep, struct file *filp,
...
static int __devinit at24c02_probe(struct i2c_client *client,
